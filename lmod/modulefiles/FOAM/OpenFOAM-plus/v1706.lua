...
local wm_cxxflags="-O3 -fPIC -std=c++11"
local wm_ldflags="-O3"
...
local wm_compiler_type="system"
local wm_compiler_lib_arch=wm_arch_option
local wm_compile_option="Opt"

local wm_precision_option="DP"
local wm_label_size="32"
local wm_label_option="Int"..wm_label_size
local wm_mplib="SYSTEMOPENMPI"
local wm_ostype="POSIX"
local wm_options=wm_arch..wm_compiler..wm_precision_option..wm_label_option..wm_compile_option

local wm_link_language="c++"
local wm_ncompprocs=capture("expr `nproc` / 2")

local boost_version="1_64_0"
local boost_arch_path=pathJoin(wm_third_party_dir, "platforms",wm_arch..wm_compiler, "boost_"..boost_version)
local cgal_version="4.10"
local cgal_arch_path=pathJoin(wm_third_party_dir, "platforms", wm_arch..wm_compiler, "CGAL-"..cgal_version)
local metis_version="5.1.0"
local metis_arch_path=pathJoin(wm_third_party_dir, "platforms", wm_arch..wm_compiler..wm_precision_option..wm_label_option, "metis-"..metis_version)
local scotch_version="6.0.3"
local scotch_arch_path=pathJoin(wm_third_party_dir, "platforms", wm_arch..wm_compiler..wm_precision_option..wm_label_option, "scotch_"..scotch_version)
local fftw_version="3.3.6-pl1"
local fftw_arch_path=pathJoin(wm_third_party_dir, "platforms", wm_arch..wm_compiler..wm_precision_option..wm_label_option, "fftw-"..fftw_version)

local foam_appbin=pathJoin(wm_project_dir, "platforms", wm_arch..wm_compiler..wm_precision_option..wm_label_option..wm_compile_option, "bin")
local foam_app=pathJoin(wm_project_dir, "applications")
local foam_etc=pathJoin(wm_project_dir, "etc")
local foam_ext_libbin=pathJoin(wm_third_party_dir, "platforms", wm_arch..wm_compiler..wm_precision_option..wm_label_option, "lib")
local foam_inst_dir=wm_project_inst_dir
local foam_job_dir=pathJoin(wm_project_inst_dir, "jobControl")
local foam_libbin=pathJoin(wm_project_dir, "platforms", wm_arch..wm_compiler..wm_precision_option..wm_label_option..wm_compile_option, "lib")
local foam_mpi="mpi"
local foam_run=pathJoin(wm_project_user_dir, "run")
local foam_settings=""
local foam_sigfpe=""
local foam_site_appbin=pathJoin(wm_project_inst_dir, "site", version, "platforms", wm_arch..wm_compiler..wm_precision_option..wm_label_option..wm_compile_option, "bin")
local foam_site_libbin=pathJoin(wm_project_inst_dir, "site", version, "platforms", wm_arch..wm_compiler..wm_precision_option..wm_label_option..wm_compile_option, "lib")
...
local foam_user_appbin=pathJoin(wm_project_user_dir, "platforms", wm_arch..wm_compiler..wm_precision_option..wm_label_option..wm_compile_option, "bin")
local foam_user_libbin=pathJoin(wm_project_user_dir, "platforms", wm_arch..wm_compiler..wm_precision_option..wm_label_option..wm_compile_option, "lib")
local foam_utilities=pathJoin(wm_project_dir, "applications/utilities")
local foamy_hex_mesh="no"
local lib_src=pathJoin(wm_project_dir, "src")
...
setenv("WM_COMPILER_TYPE", wm_compiler_type)
setenv("WM_COMPILER_LIB_ARCH", wm_arch_option)
setenv("WM_COMPILE_OPTION", wm_compile_option)

setenv("WM_PRECISION_OPTION", wm_precision_option)
setenv("WM_LABEL_SIZE", wm_label_size)
setenv("WM_LABEL_OPTION", wm_label_option)
...
setenv("METIS_VERSION", metis_version)
setenv("METIS_ARCH_PATH", metis_arch_path)
setenv("SCOTCH_VERSION", scotch_version)
setenv("SCOTCH_ARCH_PATH", scotch_arch_path)
setenv("FFTW_VERSION", fftw_version)
setenv("FFTW_ARCH_PATH", fftw_arch_path)

setenv("FOAM_APPBIN", foam_appbin)
setenv("FOAM_APP", foam_app)
setenv("FOAM_ETC", foam_etc)
...
setenv("FOAMY_HEX_MESH", foamy_hex_mesh)
setenv("LIB_SRC", lib_src)
...
prepend_path("LD_LIBRARY_PATH", pathJoin(foam_ext_libbin))
prepend_path("LD_LIBRARY_PATH", pathJoin(foam_libbin))
prepend_path("LD_LIBRARY_PATH", pathJoin(foam_site_libbin))
prepend_path("LD_LIBRARY_PATH", pathJoin(foam_user_libbin))
